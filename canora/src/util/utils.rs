//! Core utility functions used pervasively across canora.
//!
...
pub fn valid_audio(y: ArrayView1<Float>) -> Result<bool> {
    if y.is_empty() {
        return Err(CanoraError::InvalidAudio("audio buffer is empty".into()));
    }
    for (i, &v) in y.iter().enumerate() {
        if !v.is_finite() {
            return Err(CanoraError::InvalidAudio(format!(
...
pub fn expand_to(x: ArrayView1<Float>, ndim: usize, axes: i32) -> Result<crate::types::ArrayDynFloat> {
    ...
    .map_err(|e| CanoraError::ShapeMismatch {
